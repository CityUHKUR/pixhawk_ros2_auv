...
class PixhawkModule(Node):
    ...
    def listener_callback(self, msg):
        veclist = msg.split()
        vecx = self.map_to_range(float(veclist[0]), -1,  1, -1000,  1000)
        vecy = self.map_to_range(float(veclist[1]), -1,  1, -1000,  1000)
        vecz = self.map_to_range(float(veclist[2]), -1,  1,  0,  1000)
        vecr = self.map_to_range(float(veclist[3]), -1,  1, -1000,  1000)
